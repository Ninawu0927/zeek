...
void Func::DescribeDebug(ODesc* d, const std::vector<IntrusivePtr<Val>>& args) const
	{
	d->Add(Name());
...
IntrusivePtr<Val> Func::Call(val_list* args, Frame* parent) const
	{
	std::vector<IntrusivePtr<Val>> a;
	a.reserve(args->length());

	for ( auto& v : *args )
		a.emplace_back(AdoptRef{}, v);

	return Call(std::move(a), parent);
	}

IntrusivePtr<Val> BroFunc::Call(std::vector<IntrusivePtr<Val>> args, Frame* parent) const
	{
...
IntrusivePtr<Val> BuiltinFunc::Call(std::vector<IntrusivePtr<Val>> args,
                                    Frame* parent) const
	{
#ifdef PROFILE_BRO_FUNCTIONS
	DEBUG_MSG("Function: %s\n", Name());
