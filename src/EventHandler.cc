...
void EventHandler::Call(std::vector<IntrusivePtr<Val>> vl, bool no_remote)
	{
	...
	if ( local )
		...
		local->Call(std::move(vl));
	}

void EventHandler::NewEvent(const std::vector<IntrusivePtr<Val>>& vl)
	{
	if ( ! new_event )
		return;
